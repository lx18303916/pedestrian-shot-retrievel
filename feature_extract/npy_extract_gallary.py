...
# 测试图片
# probe_path = os.path.join("E:\\python_prj\\Video-Person-Search\\yolo_detection\\bbox_datasets","train.txt")
...
droprate = 1
stride = 2
# net = make_model(reidCfg, num_class=751, camera_num=0, view_num=0)
...
# probe_ft = []
# f = open(probe_path, 'r')
shot_root = "E:\\python_prj\\yolov5-master\\runs\\detect\\exp_2\\crops\\person"
sum = 0

for root1, dirs_shot,files in os.walk(shot_root):
    #遍历每个镜头文件夹
    for dir in dirs_shot:
        shot_dir = os.path.join(root1, dir)
        for root2, dirs_inshot, files_inshot in os.walk(shot_dir):
            # 遍历每个镜头文件夹的图片
            gallery_ft = []
            for file in files_inshot:
                if(file.endswith("jpg")):
                    file_path = os.path.join(root2, file)
                    start_time = time.time()
                    temp_ft = extract_ft(file_path)
                    end_time = time.time()
                    seconds = (end_time - start_time)
                    sum += seconds
                    gallery_ft.append([temp_ft, file_path])
                # print(file_path)
            gallery_ft = np.array(gallery_ft, dtype=object)
            file_name = "galleryft_" + dir + ".npy"
            savepath = os.path.join(root1, "shot_feature", file_name)
            np.save(savepath, gallery_ft)
